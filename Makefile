...
#-----------------------------------------------------------------------------
vendor_saved_apps := AntHalService Bluetooth KeyChain HTMLViewer UserDictionaryProvider BackupRestoreConfirmation \
                     FusedLocation PrintSpooler SharedStorageBackup  ExternalStorageProvider InputDevices \
                     SharedStorageBackup ProxyHandler Shell DefaultContainerService Stk \
                     WAPPushManager CMSettingsProvider
...
#-----------------------------------------------------------------------------
board_remove_apps := LogReport NfcNci GameCenter AppCenter Life \
                     Reader EBook VoiceAssistant AliAppAccount IflytekSpeechService \
                     MzSyncService Email Music Tag Camera MzUpdate \
...
override_property += \
    ...
    ro.flyme.romer=Dinphy \
    ro.product.model_romer=RedMi-3_Dinphy
